...
def score(l1, l2):
    return 10 * len((set(l1)).difference(set(l2))) + len((set(l1)).symmetric_difference(set(l2))) - 20 * len((set(l1)).intersection(set(l2)))
...
def search_data():
    ...
    schedules = sorted(list(schedules_set))
    return json.dumps([str(s[1]) for s in schedules[:5]])
